...
Prerequisites: "{input_folder}/merged_node_mapping_{start_year}_{end_year}.csv.gz", which contains the 
...
registered at any address on this date. It creates a flag stored in the "active" column of the 
...
Output:
-------
    * {output_folder}\\nodelists\\base_start_{start_year}_end_{end_year}_year_{year}.csv.gz
        * label
...
Usage:
------
    /c/mambaforge/envs/9629/python.exe 02_nodes_base_files.py 2009 2023 2023 /h/shared_data/nodelists
...
for year in `seq 2010 2023`
do
    /c/mambaforge/envs/9629/python.exe 02_nodes_base_files.py 2009 2023 $year /h/shared_data/nodelists
done
"""
...
merged_nodes = pl.read_csv(
    f"{input_folder}/merged_node_mapping_{start_year}_{end_year}.csv.gz",
    has_header=True
)
...
output = f'{output_folder}\\base_start_{start_year}_end_{end_year}_year_{year}.csv'
