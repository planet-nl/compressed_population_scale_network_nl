...
Usage example (bash):
---------------------
/c/mambaforge/envs/9629/python.exe 01_nodes_merged_nodelist.py 2009 2023 /h/shared_data/nodelists
...
Output:
-------
{output_folder}/merged_node_mapping_{start_year}_{end_year}.csv.gz
...
# save
print(f"Saving merged dataframe to " + os.path.join(output_folder,f"merged_node_mapping_{start_year}_{end_year}.csv.gz..."))
merged_node_df.to_csv(os.path.join(output_folder,f"merged_node_mapping_{start_year}_{end_year}.csv.gz"),header=True,index=False,compression="gzip")
print("Done.")
